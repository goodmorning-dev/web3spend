...
describe('deleteAllData', () => {
  ...
  it('leaves every table untouched if any one clear fails, instead of a partial delete', async () => {
    await db.cards.put({ id: 'card-1', last4: '1234', cardHolderKey: 'jane doe', label: 'Card' })
    await db.transactions.put({
      id: 'txn-1',
      cardId: 'card-1',
    ...
    // simulate a failure partway through, after cards.clear() would have
    // already committed if each table were cleared in its own transaction
    const clearSpy = vi.spyOn(db.settings, 'clear').mockRejectedValueOnce(new Error('boom'))

    await expect(deleteAllData()).rejects.toThrow('boom')
