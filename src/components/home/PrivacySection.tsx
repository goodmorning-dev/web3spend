 ...
 * it from one to the next: down on phones, where the steps stack, and
 * across from `sm` up. It stays still, parked halfway, for anyone who has
 * asked their system for reduced motion.
 */
function FlowConnector({ delay }: { delay: string }) {
  return (
    <span className="relative h-8 w-px shrink-0 bg-positive/30 sm:h-px sm:w-6 lg:w-8">
      <span
        className="absolute top-1/2 left-1/2 size-1.5 -translate-1/2 rounded-full bg-positive shadow-[0_0_8px] shadow-positive motion-safe:animate-[flow-down_1.8s_linear_infinite] sm:motion-safe:animate-[flow-across_1.8s_linear_infinite]"
    ...
    >
      ...
      <div className="flex w-full max-w-[240px] flex-col gap-2 rounded-xl border border-white/10 bg-[#0d1016] p-3 sm:w-32">
        <span className="flex items-center gap-1.5 text-[11px] font-semibold">
          <FileSpreadsheet className="size-3.5 shrink-0 text-positive" />
          export.xlsx
        </span>
...
/**
 * What "100% private" actually means, point by point (#43), in the page's
 * second accent color: green marks privacy, gold stays for actions.
 */
function PrivacySection() {
  return (
    <section
      aria-labelledby="privacy"
      className="relative isolate overflow-hidden rounded-3xl border border-positive/15 bg-gradient-to-b from-positive/[0.07] to-transparent p-6 sm:p-10"
    >
      <div
        aria-hidden="true"
      ...
      <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-[1fr_1.1fr]">
        <div className="flex flex-col items-start gap-3">
          ...
          <h2 id="privacy" className="font-heading text-2xl font-semibold sm:text-3xl">
            Private by design, <span className="text-positive">not by promise</span>.
          </h2>
          <p className="max-w-md text-base text-text-dim">
            &quot;100% private&quot; isn&apos;t a setting you have to trust. It&apos;s how Web3Spend
            is built: there&apos;s no server that ever receives your transactions.
          </p>
          <a
            href={REPO_URL}
            target="_blank"
